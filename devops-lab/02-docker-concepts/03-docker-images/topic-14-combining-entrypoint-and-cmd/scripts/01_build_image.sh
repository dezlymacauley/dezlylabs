...
-t dezlymacauley/combining-entrypoint-and-cmd:0.0.1 \
-f ../Dockerfile ./
...
# This tells Docker where to find the file that contains the 
...
# The last part of the command is called the `build context` 
# This directs Docker to the root of the local project directory.

# to start looking for all the files 
# and directories that it needs to build the Docker image.

# Docker to use all the files and directories in this directory
# to build the image.

# If you want certain files and sub-directories to be ignored,
